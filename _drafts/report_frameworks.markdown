...
Instead of putting business logic in our View layer I like to do this:
...
### Data archiving
Even with declining storage costs you eventually need to start purging some of the granular data and only retain high level summary stats.  I personally like to pregenerate data in all granularity levels upfront and then delete the more granular data sets.  

Another option is to rollup details stats to daily summaries (deleting detailed stats), then convert daily to monthly (and delete daily records).  
