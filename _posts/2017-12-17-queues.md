...
RabbitMQ supports powerful message routing via exchange (our application sends one message).  This is very important when we need to run the same job on a specific server, group of servers or all servers.  RabbitMQ also has vhosts so that multiple applications can share the same RabbitMQ server but be isolated from each other (we can create unique logins for separate applications to access their vhosts).  RabbitMQ can be setup in clusters for redundancy / failover and will acknowledge receipt of messages.  
