...
What if we had a blogging platform system where we are tracking [unique visitors](https://en.wikipedia.org/wiki/Unique_user#Unique_visitor)?  We want to give different experience to new vs. returning visitors.  For example, [comScore](https://www.comscore.com) tracks unique monthly visitors using combination of IP and [user agent](https://en.wikipedia.org/wiki/User_agent).  In a preivous job I helped built very similar functionality.  

We can take IP & UserAgent and hash the string using [MurmurHash](https://en.wikipedia.org/wiki/MurmurHash) with [digest-murmurhash gem](https://github.com/ksss/digest-murmurhash).  
...
We are using [Rails helpers](https://mixandgo.com/blog/the-beginners-guide-to-rails-helpers) but alternatively code could be moved into service objects or models.  
...
And we could use a background job.  Our application allows internal admins to fetch data from remote server and update cached content in Redis.
